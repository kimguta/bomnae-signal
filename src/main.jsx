...
function DistrictSvg({features,onSelect}){
 ...
 const markerScale=view.w/W;
 return <svg ref={svgRef} className="district-svg" viewBox={`${view.x} ${view.y} ${view.w} ${view.h}`} onWheel={wheel} onPointerDown={down} onPointerMove={move} onPointerUp={up} onPointerCancel={up} role="img">{features.map(f=>{const [x,y]=project(f.properties.center),fontSize=14*markerScale;return <g key={f.properties.code} className={`district ${f.signal.type}`} onClick={()=>{if(!suppressClick.current)onSelect(f);suppressClick.current=false}}><path d={geometryPath(f)}/><circle cx={x} cy={y} r={9*markerScale}/><text x={x} y={y-16*markerScale} style={{fontSize}}>{f.properties.name}</text><text className="weather-glyph" x={x} y={y+4*markerScale} style={{fontSize:13*markerScale}}>{f.signal.icon}</text></g>})}</svg>
}
function App(){
 const now=new Date();
